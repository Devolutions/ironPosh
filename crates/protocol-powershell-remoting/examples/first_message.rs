...
use base64::Engine;
use protocol_powershell_remoting::{PowerShellFragment, PsObject};
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let message = fragmented.blob;

    let mut cursor = std::io::Cursor::new(message);

    let pwsh_remoting_message =
        protocol_powershell_remoting::PowerShellRemotingMessage::parse(&mut cursor)?;

    println!("{:?}", pwsh_remoting_message);

    let parsed_string_data = str::from_utf8(pwsh_remoting_message.data)?;

    println!("Parsed Data: {}", parsed_string_data);

    let xml_representation = xml::parser::parse(parsed_string_data)?;

    let root_element = xml_representation.root_element();

    let ps_object = PsObject::from_node(root_element)?;

    println!("Parsed PSObject: {:?}", ps_object);
