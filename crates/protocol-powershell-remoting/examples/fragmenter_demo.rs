use protocol_powershell_remoting::{
    DefragmentResult, Defragmenter, Destination, Fragment, Fragmenter, MessageType,
    PowerShellRemotingMessage, PsObject,
};
...
fn main() {
    ...
    for (i, fragment) in fragments.iter().enumerate() {
        println!(
            "   Fragment {}: object_id={}, fragment_id={}, start={}, end={}, size={}",
            i,
            fragment.object_id,
    ...
    let all_fragments: Vec<&Fragment> = request_groups
        ...
        .collect();
    println!(
        "   {} messages produced {} fragments in {} request groups",
        messages.len(),
        all_fragments.len(),
        request_groups.len()
    );
    ...
    let all_fragments: Vec<&Fragment> = request_groups
        ...
        .collect();
    println!(
        "   Original {} messages -> {} fragments in {} request groups",
    ...
    match defragmenter.defragment(&wire_data) {
        Ok(DefragmentResult::Complete(recovered_messages)) => {
            println!(
                "   Successfully recovered {} messages",
            ...
            for (i, (original, recovered)) in original_messages
                .iter()
                .zip(recovered_messages.iter())
                .enumerate()
            {
    ...
    for (i, fragment) in fragments.iter().enumerate() {
        ...
        match partial_defragmenter.defragment(&wire_data) {
            Ok(DefragmentResult::Complete(messages)) => {
                println!(
                    "   After fragment {}: {} complete messages, {} buffered objects",
                    i + 1,
                    messages.len(),
                    partial_defragmenter.pending_count()
                );
            ...
            Ok(DefragmentResult::Incomplete) => {
                println!(
                    "   After fragment {}: 0 complete messages, {} buffered objects",
                    i + 1,
                    partial_defragmenter.pending_count()
                );
            }
