use crate::{deserialize::PsXmlDeserialize, messages::{deserialize::DeserializationContext, PsValue}};
...
fn test_parse_real_pipeline_output() {
    ...
    assert!(complex_obj.type_def.is_some(), "Should have type definition");
    
    let type_def = complex_obj.type_def.as_ref().unwrap();
    assert!(!type_def.type_names.is_empty(), "Should have type names");
    assert_eq!(type_def.type_names[0], "System.IO.DirectoryInfo", "First type should be DirectoryInfo");
    
    // Test ToString
    assert_eq!(complex_obj.to_string, Some("ADMF".to_string()), "ToString should be ADMF");

    // Test adapted properties
    assert!(!complex_obj.adapted_properties.is_empty(), "Should have adapted properties");
    
    // Check specific properties
    let name_prop = complex_obj.adapted_properties.get("Name")
    ...
    let full_name_prop = complex_obj.adapted_properties.get("FullName")
        .expect("Should have FullName property");
    if let PsValue::Primitive(crate::messages::PsPrimitiveValue::Str(full_name)) = &full_name_prop.value {
        assert_eq!(full_name, "C:\\Users\\Administrator\\Documents\\ADMF", "FullName should match");
    } else {
        panic!("FullName property should be a string");
    }

    let exists_prop = complex_obj.adapted_properties.get("Exists")
    ...
    assert!(!complex_obj.extended_properties.is_empty(), "Should have extended properties");
    
    let ps_path_prop = complex_obj.extended_properties.get("PSPath")
    ...
    if let PsValue::Primitive(crate::messages::PsPrimitiveValue::Str(ps_path)) = &ps_path_prop.value {
        assert_eq!(ps_path, "Microsoft.PowerShell.Core\\FileSystem::C:\\Users\\Administrator\\Documents\\ADMF", "PSPath should match");
    } else {
        panic!("PSPath property should be a string");
    }

    // Test nested objects (PSDrive)
    let ps_drive_prop = complex_obj.extended_properties.get("PSDrive")
        .expect("Should have PSDrive property");
    if let PsValue::Object(ps_drive_obj) = &ps_drive_prop.value {
        assert!(ps_drive_obj.type_def.is_some(), "PSDrive should have type definition");
        let ps_drive_type = ps_drive_obj.type_def.as_ref().unwrap();
        assert_eq!(ps_drive_type.type_names[0], "System.Management.Automation.PSDriveInfo", "PSDrive type should be correct");
        
        // Test PSDrive ToString
        assert_eq!(ps_drive_obj.to_string, Some("C".to_string()), "PSDrive ToString should be C");
        
        // Test PSDrive properties
        let name_prop = ps_drive_obj.adapted_properties.get("Name")
    ...
    let ps_provider_prop = complex_obj.extended_properties.get("PSProvider")
        .expect("Should have PSProvider property");
    if let PsValue::Object(ps_provider_obj) = &ps_provider_prop.value {
        let drives_prop = ps_provider_obj.adapted_properties.get("Drives")
            .expect("PSProvider should have Drives property");
        if let PsValue::Object(drives_obj) = &drives_prop.value {
            if let crate::messages::ComplexObjectContent::Container(crate::messages::Container::List(drives_list)) = &drives_obj.content {
                ...
                if let PsValue::Object(ref_obj) = &drives_list[0] {
                    ...
                    assert_eq!(ref_obj.to_string, Some("C".to_string()), "Referenced PSDrive should have ToString C");
                } else {
    ...
    println!("Object type: {:?}", complex_obj.type_def.as_ref().unwrap().type_names[0]);
    println!("ToString: {:?}", complex_obj.to_string);
    println!("Adapted properties count: {}", complex_obj.adapted_properties.len());
    println!("Extended properties count: {}", complex_obj.extended_properties.len());
}
    ...
    match &ps_value {
        PsValue::Object(obj) => {
            ...
            match &obj.content {
                ...
                crate::messages::ComplexObjectContent::ExtendedPrimitive(prim) => println!("\nContent: ExtendedPrimitive({:?})", prim),
                crate::messages::ComplexObjectContent::Container(container) => {
                    println!("\nContent: Container({:?})", container);
                }
                crate::messages::ComplexObjectContent::PsEnums(enums) => println!("\nContent: Enum({})", enums.value),
            }
        }
...
fn classify_ps_value(value: &PsValue) -> String {
    match value {
        ...
        PsValue::Object(obj) => {
            let type_name = obj.type_def.as_ref()
            ...
            format!("Object({}, content: {}, props: {}, ext_props: {})", 
                type_name, content_type, obj.adapted_properties.len(), obj.extended_properties.len())
        }
    }
}
