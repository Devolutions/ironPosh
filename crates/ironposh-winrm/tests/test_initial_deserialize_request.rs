...
mod tests {
    ...
    fn test_deserialize_soap_headers() {
        let node =
            ironposh_xml::parser::parse(SOAP_HEADER_XML).expect("Failed to parse XML string");
    ...
    fn test_xml_parsing_success() {
        ...
        let parse_result = ironposh_xml::parser::parse(SOAP_HEADER_XML);
    ...
    fn test_deserialize_with_minimal_header() {
        ...
        let node =
            ironposh_xml::parser::parse(MINIMAL_HEADER).expect("Failed to parse minimal XML");
        let envelope = node.root_element();
